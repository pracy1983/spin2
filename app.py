...
def lista_alunos():
    try:
        ...
        response = supabase.table('alunos').select('*').order('nome').execute()
        alunos = response.data

        # Garantir que todas as URLs de fotos estão corretas
        for aluno in alunos:
            if aluno.get('foto_url'):
                # Se a URL não começa com /static, corrigir
                if not aluno['foto_url'].startswith('/static'):
                    filename = aluno['foto_url'].split('/')[-1]
                    aluno['foto_url'] = url_for('static', filename=f'uploads/{filename}')
            else:
                aluno['foto_url'] = url_for('static', filename='default-avatar.jpg')
...
def api_lista_alunos():
    try:
        ...
        print(f"Erro ao listar alunos via API: {str(e)}")
...
def atualizar_foto(id):
    try:
        ...
        if not allowed_file(foto.filename):
            return jsonify({'success': False, 'error': 'Tipo de arquivo não permitido. Use apenas .jpg, .jpeg, .png ou .gif'}), 400
        ...
        filename = secure_filename(f"aluno_{id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        ...
        if processed_image:
            ...
            return jsonify({'success': False, 'error': 'Erro ao processar a imagem'}), 400

        # Atualizar no banco
        supabase = create_client(os.getenv('SUPABASE_URL'), os.getenv('SUPABASE_ANON_KEY'))
        result = supabase.table('alunos').update(dados).eq('id', id).execute()
        
        return jsonify({
            'success': True, 
            'message': 'Foto atualizada com sucesso',
            'foto_url': url_for('uploaded_file', filename=processed_filename)
        })
...
def cadastrar_aluno():
    try:
        ...
        dados = {
            'nome': request.form.get('nome') or '*mudar*',
            'email': request.form.get('email') or '*mudar*',
            'telefone': request.form.get('telefone') or '*mudar*',
            'data_nascimento': request.form.get('data_nascimento') or '2000-01-01',
            ...
            'data_inicio_desenvolvimento': request.form.get('data_inicio_desenvolvimento') or None,
            'data_inicio_estagio': request.form.get('data_inicio_estagio') or None,
            'data_iniciacao_magista': request.form.get('data_iniciacao_magista') or None,
            'data_iniciacao_mestre': request.form.get('data_iniciacao_mestre') or None,
            'data_inicio_not': request.form.get('data_inicio_not') or None,
            'fundador': request.form.get('fundador') == 'on'
        }
        ...
        if 'foto' in request.files:
            ...
            if foto and foto.filename and allowed_file(foto.filename):
                ...
                unique_filename = f"aluno_{int(time.time())}{ext}"
                ...
                # Processar a foto para formato 3x4
                with Image.open(filepath) as img:
                    img.thumbnail((800, 1067))
                    img.save(filepath, quality=85, optimize=True)
                
                dados['foto_url'] = url_for('static', filename=f'uploads/{unique_filename}')
        ...
        if response.data:
            ...
            return jsonify({'success': False, 'error': 'Erro ao cadastrar aluno'}), 400
...
def atualizar_foto_api(id):
    try:
        ...
        # Gerar nome único para o arquivo
        filename = secure_filename(f"{id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        
        # Salvar arquivo
        foto.save(filepath)
        
        # Processar a foto (redimensionar, etc)
        ...
        if processed_img:
            ...
            # Atualizar no banco
            supabase = create_client(os.getenv('SUPABASE_URL'), os.getenv('SUPABASE_ANON_KEY'))
            result = supabase.table('alunos').update({'foto_url': foto_url}).eq('id', id).execute()
            ...
            os.remove(filepath)  # Remove o arquivo se falhou no processamento
            return jsonify({'success': False, 'error': 'Erro ao processar a imagem'}), 500
...
@app.route('/api/aluno/<int:aluno_id>/foto', methods=['POST'])
@login_required
def upload_foto(aluno_id):
    ...
    try:
        ...
        filename = secure_filename(foto.filename)
        ext = os.path.splitext(filename)[1]
        unique_filename = f"aluno_{aluno_id}_{int(time.time())}{ext}"
        ...
        try:
            ...
            # Atualiza o URL da foto no banco de dados
            ...
            if not response.data:
                raise Exception('Erro ao atualizar foto no banco de dados')
            ...
            # Se houver erro no processamento, remove o arquivo
            if os.path.exists(filepath):
                os.remove(filepath)
            raise img_error

    except Exception as e:
        app.logger.error(f"Erro ao processar foto: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500

def corrigir_urls_fotos():
    try:
        ...
        for aluno in alunos:
            if aluno.get('foto_url'):
                ...
                old_url = aluno['foto_url']
                filename = old_url.split('/')[-1]
                
                # Criar nova URL com o caminho correto
                new_url = url_for('static', filename=f'uploads/{filename}')
        ...
        print(f"Erro ao corrigir URLs das fotos: {str(e)}")
        return False
...
def atualizar_status_alunos():
    try:
        ...
        for aluno in alunos:
            status = calcular_status_aluno(aluno)
...
def calcular_status_aluno(aluno):
    # Se não tiver data de início, está em desenvolvimento
    if not aluno.get('data_inicio_desenvolvimento'):
        return 'Em Desenvolvimento'
    
    # Se tiver data de iniciação como mestre, é mestre
    if aluno.get('data_iniciacao_mestre'):
        return 'Mestre'
    
    # Se tiver data de iniciação como magista, é magista
    if aluno.get('data_iniciacao_magista'):
        return 'Magista'
    
    # Se tiver data de início do estágio, é estagiário
    if aluno.get('data_inicio_estagio'):
        return 'Estagiário'
    
    # Se só tiver data de início de desenvolvimento, está em desenvolvimento
    return 'Em Desenvolvimento'
...
if __name__ == '__main__':
    ...
    try:
        ...
        corrigir_urls_fotos()  # Corrigir URLs das fotos existentes
    except Exception as e:
